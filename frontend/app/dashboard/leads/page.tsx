...
import {
  ...
  ChevronRight, Wifi, WifiOff, Clock
} from 'lucide-react';
...
export default function LeadsPage() {
  ...
  const selectStyle = {
    ...
    color: 'var(--text-secondary)', borderRadius: '1rem', padding: '1rem 1.5rem',
  ...
  return (
    <div className="space-y-8 pb-20 animate-fade-in">
      ...
      {showDeleteAll && (
        <Modal>
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto"
            style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', boxShadow: '0 4px 15px rgba(239,68,68,0.1)' }}>
            ...
            <div className="max-h-52 overflow-y-auto space-y-2 custom-scrollbar">
              ...
              {availableSessions.map(s => (
                <label key={s.session_id} className="flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all"
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-hover)'; e.currentTarget.style.borderColor = 'var(--border-glow)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'var(--bg-hover)'; e.currentTarget.style.borderColor = 'var(--border-subtle)'; }}>
                  ...
                  <div>
                    <p className="font-black text-[var(--text-primary)] text-sm capitalize">{s.session_id.replace('_', ' ')}</p>
  ...
  return (
    <div className="glass-card glass-card-hover rounded-3xl p-8 relative overflow-hidden animate-fade-in"
      ...
      <div className="absolute top-0 right-0 w-48 h-48 rounded-full opacity-5 pointer-events-none"
        style={{ background: `radial-gradient(circle, ${isHot ? '#f59e0b' : isWarm ? '#7c3aed' : '#b3acd8'}, transparent)` }} />
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start lg:items-center">
          ...
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white shrink-0 relative overflow-hidden"
            style={{
              background: isHot
                ...
                : 'var(--bg-hover)',
              border: `1px solid ${isHot ? 'rgba(245,158,11,0.3)' : isWarm ? 'var(--border-bright)' : 'var(--border-glow)'}`,
            }}>
          ...
          <div>
            ...
            {lead.company && (
              <div className="flex items-center gap-2 mt-1">
                <Building2 size={13} style={{ color: 'var(--text-ghost)' }} />
                <p className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--text-secondary)' }}>{lead.company}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 lg:ml-auto">
          ...
          <button onClick={onDelete}
            className="w-12 h-12 rounded-2xl flex items-center justify-center transition-all animate-float"
      ...
      <div className="mt-6 pt-6 flex flex-col md:flex-row justify-between items-center gap-4"
        ...
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-ghost)' }}>
          <Calendar size={13} />
